...
func Timer() {
	...
	id, _ = global.Timer.AddTaskByFunc("test5", "25-29 9 * * 1-5", func() {
		for i := 0; i < 18; i++ {
			service.WenSearchBiddingData("上个交易日板块热度前4；非同花顺特色指数；非同花顺地域概念；上个交易日涨停家数>8","涨跌幅大于0%且涨跌幅小于30%；量比大于2；委比大于50%；上市天数大于1年；流通市值小于300亿；非st的股票；macd零轴上；近1年涨跌幅小于50%；近10个交易日涨幅大于10%且小于40%；同花顺行业；不包含创业板股票；")
